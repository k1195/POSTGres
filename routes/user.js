...
const client = require('../middleware/redi')
  ...
  router.post('/newuser', async (req,res)=>{
   ...
   try {
    ...
    .create({ firstName: req.body.firstName , lastName: req.body.lastName , uId : req.body.uId})
    // .then(function(result) {
    //   console.log(result)
    const token = jwt.sign({_id:userCreate.uId},JWT_SECRET)
    res.status(200).json({token,userCreate})  
    // })
     ...
     try {
        ...
        if(req.body.uId ){
          ...
          if(value){
            ...
            .findOne({
             where: {uId:req.body.uId},raw:true,nest:true})
